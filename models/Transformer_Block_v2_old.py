    ...
    def forward(self, x, edge_index, batch_size = 1, use_softmax=False, return_attn_matrix=False):
        ...
        if use_softmax:
            ...
            for b in range(batch_size):
                adj_matrix[b] = to_dense_adj(edge_index)
            ...
            attn_matrix = torch.Tensor(batch_size, num_nodes, num_nodes).to(self.device)
            for b in range(batch_size):
                attn_matrix[b] = to_dense_adj(edge_index, edge_attr=attn_weights[b].reshape(-1))
        
        ## Calculate final output
        x = torch.einsum("bij,bih->bjh", attn_matrix, x2)
