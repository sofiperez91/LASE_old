...
MODEL_FILE='../saved_models/glase_unshared_d2_normalized_full_negative_phase2_init.pt'
TRAIN_DATA_FILE='./data/sbm2_neg_train_2.pkl'
VAL_DATA_FILE='./data/sbm2_neg_val_2.pkl'
# TRAIN_DATA_FILE='./data/sbm2_train.pkl'
# VAL_DATA_FILE='./data/sbm2_val.pkl'
...
# with open('./data/x_rand_init.pkl', 'wb') as f:
#     pickle.dump(x, f)
...
for epoch in range(epochs):
    ...
    for i, batch in enumerate(train_loop):  
        ...
        # x = get_x_init(num_nodes, -math.pi/2, math.pi/2).to(device)  
        # x = get_x_init(num_nodes, -math.pi/2, math.pi/2).to(device)
        # x = torch.nn.init.xavier_uniform_(torch.Tensor(num_nodes, d)).to(device)
    ...
    for i, batch in enumerate(val_loop):
        ...
        x = get_x_init(num_nodes, 0, math.pi/2).to(device)        
        # x = get_x_init(num_nodes, -math.pi/2, math.pi/2).to(device)     
        # x = get_x_init(num_nodes, 0, math.pi).to(device)
        # x = torch.nn.init.xavier_uniform_(torch.Tensor(num_nodes, d)).to(device)
        out = model2(x, batch.edge_index, edge_index_2, Q, mask)
        loss = torch.norm((out@Q@out.T - to_dense_adj(batch.edge_index).squeeze(0)))
